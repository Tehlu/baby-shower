...
app.post('/upload', upload.single('file'), async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (!drive) {
      res.status(500).json({ error: 'Google Drive not configured. Service account key or folder ID missing.' });
      return;
    }
    if (!process.env.GOOGLE_DRIVE_FOLDER_ID) {
      res.status(500).json({ error: 'Google Drive folder ID not set in environment variables.' });
      return;
    }
